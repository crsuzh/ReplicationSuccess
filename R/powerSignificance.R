...
.powerSignificance_ <- function(
    ...
    ## when strict == TRUE, add probability in the other
    ## direction for "two.sided"
...
#' on the evidence in the original study (with an empirical Bayes
#' shrinkage estimator).
#' @param alternative Either "one.sided" (default) or "two.sided".
#' Specifies if the significance level is one-sided or two-sided.
#' If the significance level is one-sided, then power calculations
#' are based on a
#' one-sided assessment of significance in the direction of the
#' original effect estimates.
#' @param h The relative between-study heterogeneity, i.e., the
#' ratio of the heterogeneity
...
#' Is only taken into account if the \code{designPrior} is "conditional"
#' or "predictive".
...
#' @return The probability that a replication study yields a significant
#' effect estimate
#' in the specified direction.
#' @details \code{powerSignificance} is the vectorized version of
#' \code{.powerSignificance_}.
