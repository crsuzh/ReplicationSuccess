...
the sceptical $p$-value approach \citep{Held_etal2022}.
...
GitHub (\url{https://github.com/SamCH93/ReplicationSuccess/}, the stable version
...
\item \textit{controlled}: Ensures exact Type-I error control at level $\alpha^2$ 
...
<<>>=
levCont_c1 <- levelSceptical(level = 0.025, 
                             c = 1, alternative = "one.sided", 
                             type = "controlled")
levCont_c2 <- levelSceptical(level = 0.025, 
                             c = 2, alternative = "one.sided", 
                             type = "controlled")
...
\citet{Held_etal2022} further studied properties of the different
...
replication is successful at the golden level. The golden level 
controls the  type-I error rate for $c \geq 1$, so does not provide
exact Type-I error control for any $c$
. \citet{Held_etal2022-Fram} recently proposed 
the controlled level, which ensures exact Type-I error control for 
any value of $c$. The controlled level is adaptive and depends on $c$. 
It is for example $\Sexpr{formatPval(levCont_c1)}$ for $c = 1$ and 
$\Sexpr{formatPval(levCont_c2)}$ for $c = 2$.
...
to get in touch with us over GitHub (\url{https://github.com/SamCH93/ReplicationSuccess/}).
